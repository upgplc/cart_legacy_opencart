...
class ControllerPaymentSecureHosting extends Controller {
	...
	public function index() {
        ...
        $data['transactionamount'] = $this->currency->format($order_info['total'], $order_info['currency_code'], false, false);
		...
		foreach($products as $product){
			...
			$secuitems .= '['.$product['product_id'].'||'.$product['name'].'|'.$price.'|'.$product['quantity'].'|'.$total.']';
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/securehosting.tpl')) {
			...
			return $this->load->view('default/template/payment/securehosting.tpl', $data);
		}	
	}
